...
Two requested models lacked usable detector ONNX weights; substitutes noted below. Soylent hardcase is a local proxy. We do not fetch private Proofmark quants — train neopixel-accurate-v1 with npm run distill:accurate.

**Leakage warning:** `neopixel-accurate-v1` below was trained on this same OpenRouter+Lexica tree. Full-corpus 99% BA is not a promotion signal. Use `models/neopixel-accurate-v1/manifest.json` → `onnxHoldoutAt065` (and grow the corpus) before swapping FORENSICS_MODEL.
...
|-----:|-------|---:|----:|----:|-----------:|-----------:|--------:|------------:|-------|
| 1 | `neopixel-accurate-v1` | 99.4% | 98.8% | 100.0% | 97.9% (47/48) | 83.0 | 10.8 | 0/6 | our distill |
...
|-------|---:|-----------:|------------:|
| `neopixel-accurate-v1` | 99.4% | 83.0 | 0/6 |
...
|-------|-----------:|------|---------|---:|
| `neopixel-accurate-v1` | 0.1% | real/other | real/other | 73.4 |
...
- **Best BA @65%:** `neopixel-accurate-v1` at **99.4%** (avg **83.0 ms**/image, **10.8 s** total).
- **Fastest:** `ai-image-detect-distilled` at **38.9 ms**/image (**5.1 s** total).
- **Best hardcase FP control:** `neopixel-accurate-v1` with **0/6** hardcase false positives @65%.
