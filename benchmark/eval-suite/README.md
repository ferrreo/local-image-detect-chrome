...
| `js-ext-wasm-cascade` / `js-ext-webgpu-cascade` | Unpacked MV3 + **`onnxruntime-web`** cascade (force via `engine=ort-web`; CF always WASM) |
| `js-ext-zig-wasm-cascade` / `js-ext-zig-webgpu-cascade` | Same extension on **Zig+ORT WASM** (`engine=zig`) |
