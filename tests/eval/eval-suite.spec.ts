...
const { loadOpenRouterCorpus } = require("../../scripts/lib/corpus.mjs") as {
  loadOpenRouterCorpus: (root: string) => {
    base: string;
    images: unknown[];
    source: string;
  };
...
/** Browser engines for eval suite. Extension ships onnxruntime-web only. */
const engines = (process.env.EVAL_SUITE_BROWSER_ENGINES ?? "ort-web")
  .split(",")
  .map((s) => s.trim())
  .filter((s) => s === "ort-web" || s === "onnxruntime-web" || s === "auto");
...
async function launchExtensionContext(): Promise<{
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "neopixel-eval-"));
...
test.describe("Offline extension eval suite", () => {
  ...
  test("run browser ORT providers against eval corpus", async () => {
    test.skip(corpus.images.length === 0, "Eval corpus empty");
    ...
    try {
      for (const engine of engines) {
        for (const provider of providers) {
          ...
          const corpusBase = `http://127.0.0.1:${corpusServer.port}`;
