...
type SuiteResult = {
  kind: "neopixel-browser-eval";
  generatedAt: string;
  corpusBase: string;
  providerRequested: VisualProvider["kind"];
  providerActual: string;
  /** Runtime that served visual: onnxruntime-web | … */
  visualEngine: string;
...
async function loadCorpus(base: string, limit = 0): Promise<CorpusImage[]> {
  ...
  // Fixtures: single index.json with per-image labels.
  if (
    Array.isArray(ai.images) &&
    ai.images.some((i: { label?: string }) => i.label === "ai" || i.label === "real")
  ) {
    images = ai.images.map(
      (i: { file: string; label: "ai" | "real"; model?: string }) => ({
        file: i.file,
        label: i.label,
        model: i.model ?? null,
      }),
    ...
    const realRes = await fetch(`${root}/real-index.json`);
    if (!realRes.ok) throw new Error("real-index.json missing for corpus");
...
async function runEval() {
  ...
  try {
    ...
    suiteResultEl.textContent = JSON.stringify({
      kind: "neopixel-browser-eval-error",
      message,
    });
    suiteResultEl.removeAttribute("hidden");
  } finally {
