...
function cacheUrl(model: ModelArtifact): string {
  return `https://neopixel.local/${model.cacheKey}`;
}
    ...
    for (const model of ALL_MODELS) {
      ...
      if (existing) {
        ...
        if (digest === model.sha256) {
          ...
          // Migrate legacy relative keys to https://neopixel.local/…
      ...
      if (!model.url) {
        // Package-only artifacts (NeoPixel accurate head): seed from extension bundle.
        if (typeof chrome !== "undefined" && chrome.runtime?.getURL) {
          try {
            const packaged = await fetch(chrome.runtime.getURL(model.localPath));
            if (packaged.ok) {
              const buffer = await packaged.arrayBuffer();
              const digest = await sha256Hex(buffer);
              if (digest !== model.sha256) {
                return {
                  kind: "error",
                  message: `Checksum mismatch for packaged ${model.id}: got ${digest}`,
                };
              }
              await cache.put(
                cacheUrl(model),
                new Response(buffer, {
                  headers: {
                    ...
                    "X-NeoPixel-SHA256": digest,
                    "X-NeoPixel-Model-Id": model.id,
                  },
                }),
              );
        ...
        return {
          kind: "error",
          message: `${model.id} has no public URL — package ${model.localPath} (npm run distill:accurate && npm run setup:models && npm run build)`,
        };
      }
