#!/usr/bin/env node
/**
 * Host-side eval matrix: Node ORT CPU + Zig ORT (WebGPU/Vulkan/XNNPACK/CPU prefs).
...
const zig = await server.ssrLoadModule("/extension/src/lib/visual-zig.ts");
...
async function fusePipeline(item, visual) {
  ...
  return {
    ...
    correct:
      (fused.confidence >= threshold) === (item.label === "ai"),
    backend,
...
async function runMode(mode) {
  ...
  let meta = { mode, runtime: "host" };

  if (mode.startsWith("zig-") && !zig.zigInferBinaryExists()) {
    return {
      mode,
      skipped: true,
      reason: "truepixel-infer missing (npm run build:zig)",
    };
  }

  if (mode.startsWith("zig-")) {
    await zig.shutdownVisualZig();
    const prefer =
      mode.includes("webgpu")
        ? "WebGPU"
        : mode.includes("vulkan")
          ? "Vulkan"
          : mode.includes("xnnpack")
            ? "XNNPACK"
            : "CPU";
    const warm = await zig.warmVisualZig(prefer);
    meta = {
      ...meta,
      engine: "zig-ort",
      preferEp: warm.preferEp,
      distilledEp: warm.distilledEp,
      forensicsEp: warm.forensicsEp,
      warmMs: warm.ms,
    };
  } else if (mode.startsWith("js-node-")) {
    await nodeVisual.warmVisualNode();
    meta = { ...meta, engine: "onnxruntime-node", preferEp: "CPU", device: "cpu" };
  }
  ...
  for (const item of images) {
    ...
    } else if (mode === "zig-cpu-distilled" || mode.endsWith("-distilled")) {
      row = await fusePipeline(item, async ({ abs }) => {
        const v = await zig.classifyZigVisual({
          imagePath: abs,
          runDistilled: true,
          runForensics: false,
        });
        return {
          ...
          detail: v.detail,
          inferMs: v.inferMs,
          backend: meta.distilledEp ?? "cpu",
        };
      });
    } else if (mode === "zig-cpu-cascade") {
      row = await fusePipeline(item, async ({ abs, spectralScore, spectralFeatures }) => {
        const distilled = await zig.classifyZigVisual({
          imagePath: abs,
          runDistilled: true,
          runForensics: false,
        });
        let secondaryScore;
        let detail = distilled.detail;
        let inferMs = distilled.inferMs;
        if (
          zig.needsForensicsCascade({
          ...
          const f = await zig.classifyZigVisual({
            imagePath: abs,
            runDistilled: false,
            runForensics: true,
          });
          secondaryScore = f.secondaryScore ?? f.score;
  ...
  if (mode.startsWith("zig-")) await zig.shutdownVisualZig();
  ...
  console.log(
    ...
    scored.confusion,
    meta.distilledEp ? `ep=${meta.distilledEp}` : "",
  );
  return report;
}
