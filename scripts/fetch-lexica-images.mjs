...
const UA = "NeoPixelBenchmark/1.0 (+local eval corpus)";
...
function loadJson(file, fallback) {
  ...
  try {
    ...
    console.warn(
      `loadJson ${file}: ${err instanceof Error ? err.message : err} — using fallback`,
    );
    return fallback;
  }
}
...
function appendDistillManifest(records) {
  ...
  writeJsonAtomic(distillManifestPath, current);
}
