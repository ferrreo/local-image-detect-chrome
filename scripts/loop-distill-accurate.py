#!/usr/bin/env python3
"""
Train/eval loop for neopixel-accurate-v1 until it beats the gate.
...
OUT_ROOT = ROOT / "models" / "neopixel-accurate-loop"
BEST_DIR = ROOT / "models" / "neopixel-accurate-v1"
...
def passes(report: dict, avg_ms: float) -> bool:
    # Prefer calibrated operating point when present; fall back to @0.65.
    lex = report.get("onnxLexicaHoldoutCalibrated") or report.get(
        "onnxLexicaHoldoutAt065"
    ) or {}
    hard = report.get("onnxHardcaseCalibrated") or report.get(
        "onnxHardcaseAt065"
    ) or {}
    gate = report.get("onnxHoldoutCalibrated") or report.get("onnxHoldoutAt065") or {}
    return (
        ...
        and avg_ms <= MAX_MS
        and int((report.get("onnxLexicaHoldoutAt065") or {}).get("n") or lex.get("n") or 0)
        >= 40
    )
...
def run_one(cfg: dict, round_idx: int) -> dict:
    out = OUT_ROOT / f"round-{round_idx:02d}-h{cfg['hidden']}-a{cfg['augment_views']}-{int(time.time())%100000}"
